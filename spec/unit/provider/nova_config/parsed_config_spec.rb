...
provider_class = Puppet::Type.type(:nova_config).provider(:parsed_config)
describe provider_class do
  ...
  it 'should be able to parse comma separated values' do
  ...
  it 'should parse sections as comment' do
    record = @provider.class.parse('[DEFAULT]').first
    record[:record_type].should == :comment
  end
end
